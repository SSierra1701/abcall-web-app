...
import {
  ...
  selectIsSubmitting,
  selectServerError,
} from '../../store/signInTeamReducer';
import { SignInTeamRequestI } from '../../types/signInTeamRequestI';
import { signInTeamActions } from '../../store/signInTeamActions';

@Component({
  selector: 'sign-in-form-team-c',
...
export class SignInTeamFormComponent {
  data$: Observable<{
  ...
  constructor(private store: Store<{ state: SignInTeamStateI }>) {
  ...
  onSubmit() {
    ...
    const request: SignInTeamRequestI = {
      email: this.form.controls.email.value ?? '',
      password: this.form.controls.password.value ?? '',
    };
    this.store.dispatch(signInTeamActions.signIn({ request }));
  }
